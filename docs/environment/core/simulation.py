...
class SimulationEnv:
    ...
    def restart(self, spawn_single=None, star_mode=None, airport_name=None):
        """Wipe state and re-instantiate. Drops any active recording.

        Args set to None preserve the current value. Used by the frontend's
        Single Aircraft toggle (which restarts the sim under a different
        spawn mode) and exposed over HTTP / Pyodide as /restart.
        """
        new_single = self.spawn_single if spawn_single is None else bool(spawn_single)
        new_star = self.star_mode if star_mode is None else bool(star_mode)
        new_airport = self.airport_name if airport_name is None else str(airport_name).lower()
        self.__init__(
            radar_side=self.radar_side,
            airport_name=new_airport,
            spawn_single=new_single,
            star_mode=new_star,
            recorder=None,
        )

    def start_recording(self):
        """Attach a fresh in-memory recorder. Rows start accumulating on the
        next step(). Returns True if a recorder was attached, False if one was
        already active.
        """
        from environment.core.human_data_logger import HumanDataRecorder
        if self.recorder is not None:
            return False
        rec = HumanDataRecorder(spawn_single=self.spawn_single, in_memory=True)
        rec.start()
        self.recorder = rec
        return True

    def stop_recording(self):
        """Detach the recorder and return (csv_text, suggested_filename), or
        (None, None) if not recording. Only valid when the recorder is in
        in-memory mode (i.e. started via start_recording, not the CLI flag).
        """
        rec = self.recorder
        if rec is None:
            return None, None
        self.recorder = None
        rec.close()
        if not getattr(rec, 'in_memory', False):
            return None, None
        return rec.to_csv(), rec.suggested_filename()
