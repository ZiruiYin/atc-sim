"""bc_gmm_single_full_human — data ablation of the shipped 7-D GMM.
...
200× repeat / 25 epochs). The ONLY difference is the training data pool:
    ...
    TRAIN_SOURCES = (single, multi)   ← dagger AND distillation DROPPED

i.e. pure offline human demonstrations only — the single-plane and
multi-plane sessions, with NO DAgger corrections and NO FM-distillation
rollouts. This is the "raw BC, no interactive/synthetic augmentation"
baseline. Compared against `bc_gmm_single_full_nodistill` (adds dagger
back) and the shipped `bc_gmm_single_full` (adds distill on top), the
three isolate the marginal value of each data source. Source filtering
happens at train time over the shared cache — see
`rl_bc/config.py::train_sources` and `rl_bc/bc_gmm/train.py`.
"""

NAME = "bc_gmm_single_full_human"
FAMILY = "bc_gmm"

# Data-source ablation. Codes (rl_bc/data.py): single=0, multi=1, distill=2,
# dagger=3. Keep single + multi only; drop dagger and distill.
TRAIN_SOURCES = (0, 1)
